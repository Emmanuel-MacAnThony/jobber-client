...
const RegisterModal: FC<IModalBgProps> = ({ onClose, onToggle }): ReactElement => {
  ...
  const onRegisterUser = async (event: ChangeEvent): Promise<void> => {
    try {
  ...
  return (
    <ModalBg>
      <div className="relative top-[10%] mx-auto w-11/12 max-w-md rounded bg-white md:w-2/3">
        ...
        <div className="px-5">
          <Alert type="error" message={'Sample Alert'} />
        </div>
        ...
        {step == 1 && (
          <div className="relative px-5 py-5">
            ...
            <Button
              disabled={!userInfo.username || !userInfo.password || !userInfo.email}
